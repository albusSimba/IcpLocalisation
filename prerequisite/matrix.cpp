...
Matrix::Matrix (const int32_t m_,const int32_t n_,const FLT* val_) {
  allocateMemory(m_,n_);
  int32_t k=0;
...
Matrix::Matrix (const Matrix &M) {
  ...
  for (int32_t i=0; i<M.m; i++)
    memcpy(val[i],M.val[i],M.n*sizeof(FLT));
}
...
void Matrix::getData(FLT* val_,int32_t i1,int32_t j1,int32_t i2,int32_t j2) {
  if (i2==-1) i2 = m-1;
  if (j2==-1) j2 = n-1;
  int32_t k=0;
...
void Matrix::setVal(FLT s,int32_t i1,int32_t j1,int32_t i2,int32_t j2) {
  if (i2==-1) i2 = m-1;
  if (j2==-1) j2 = n-1;
  if (i2<i1 || j2<j1) {
...
void Matrix::setDiag(FLT s,int32_t i1,int32_t i2) {
...
Matrix Matrix::rotMatX (const FLT &angle) {
  FLT s = sin(angle);
  FLT c = cos(angle);
  Matrix R(3,3);
  R.val[0][0] = +1;
...
Matrix Matrix::rotMatY (const FLT &angle) {
  FLT s = sin(angle);
  FLT c = cos(angle);
  Matrix R(3,3);
  R.val[0][0] = +c;
...
Matrix Matrix::rotMatZ (const FLT &angle) {
  FLT s = sin(angle);
  FLT c = cos(angle);
  Matrix R(3,3);
  R.val[0][0] = +c;
...
Matrix Matrix::operator* (const FLT &s) {
  Matrix C(m,n);
  for (int32_t i=0; i<m; i++)
    for (int32_t j=0; j<n; j++)
      C.val[i][j] = val[i][j]*s;
...
bool Matrix::solve (const Matrix &M, FLT eps) {
...
bool Matrix::lu(int32_t *idx, FLT &d, FLT eps) {
  ...
  FLT* vv = (FLT*)malloc(n*sizeof(FLT)); // vv stores the implicit scaling of each row.
  d = 1.0;
...
void Matrix::svd(Matrix &U2,Matrix &W,Matrix &V) {
  ...
  FLT* w   = (FLT*)malloc(n*sizeof(FLT));
  FLT* rv1 = (FLT*)malloc(n*sizeof(FLT));

  int32_t flag,i,its,j,jj,k,l,nm;
  FLT   anorm,c,f,g,h,s,scale,x,y,z;
  ...
  for (k=n-1;k>=0;k--) { // Diagonalization of the bidiagonal form: Loop over singular values,
    for (its=0;its<30;its++) { // and over allowed iterations.
      ...
      for (l=k;l>=0;l--) { // Test for splitting.
        nm = l-1;
        if ((FLT)(fabs(rv1[l])+anorm) == anorm) { flag = 0; break; }
        if ((FLT)(fabs( w[nm])+anorm) == anorm) { break; }
      }
      if (flag) {
        ...
        for (i=l;i<=k;i++) {
          f = s*rv1[i];
          rv1[i] = c*rv1[i];
          if ((FLT)(fabs(f)+anorm) == anorm) break;
          g = w[i];
          h = pythag(f,g);
          w[i] = h;
          h = 1.0/h;
  ...
  int32_t s2,inc=1;
  FLT   sw;
  FLT* su = (FLT*)malloc(m*sizeof(FLT));
  FLT* sv = (FLT*)malloc(n*sizeof(FLT));
...
void Matrix::allocateMemory (const int32_t m_,const int32_t n_) {
  ...
  val    = (FLT**)malloc(m*sizeof(FLT*));
  val[0] = (FLT*)calloc(m*n,sizeof(FLT));
